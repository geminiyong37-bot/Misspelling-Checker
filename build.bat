@echo off
setlocal

set SCRIPT_DIR=%~dp0
set STAGING=%SCRIPT_DIR%Output\_staging
set INSTALLER_OUTPUT=%STAGING%\installer
set ISCC="C:\Inno Setup 6\ISCC.exe"

echo [1/5] PyInstaller 빌드 중 (Main App)...
pyinstaller "%SCRIPT_DIR%build.spec" --noconfirm --distpath "%STAGING%"
if errorlevel 1 (
    echo PyInstaller 실패 - Main App
    exit /b 1
)

echo [2/5] PyInstaller 빌드 중 (Key Verifier)...
pyinstaller --noconfirm --onefile --console --name verify_key "%SCRIPT_DIR%src\verify_key.py" --distpath "%STAGING%"
if errorlevel 1 (
    echo PyInstaller 실패 - Key Verifier
    exit /b 1
)

echo [3/5] 설치 파일 생성 중...
if not exist "%INSTALLER_OUTPUT%" mkdir "%INSTALLER_OUTPUT%"
%ISCC% /O"%INSTALLER_OUTPUT%" "%SCRIPT_DIR%installer.iss"
if errorlevel 1 (
    echo Inno Setup 실패
    rmdir /s /q "%STAGING%"
    exit /b 1
)

echo [4/5] 완성된 설치 파일 게시 중...
copy /y "%INSTALLER_OUTPUT%\AI_Word_Speller_Setup.exe" "%SCRIPT_DIR%Output\AI_Word_Speller_Setup.exe"
if errorlevel 1 (
    echo 설치 파일 게시 실패 - 기존 설치 파일이 열려 있는지 확인해 주세요.
    exit /b 1
)

echo [5/5] 임시 파일 정리 중...
rmdir /s /q "%STAGING%"

echo 완료: Output\AI_Word_Speller_Setup.exe
endlocal
